                ...
                if cell.is_revealed and cell.mines_around > 0:
                    ...
                    for dx in range(-1, 2):
                        for dy in range(-1, 2):
                            nx, ny = x + dx, y + dy
                            if (
                                0 <= nx < self.grid.cell_num
                                and 0 <= ny < self.grid.cell_num
                            ):
                    ...
                    if (
                        len(adj) > 0
                        and len(adj) + flagged == cell.mines_around
                        and self.top_bar.stats.num_flags > 0
                    ):
                        nx, ny = adj[0]
                        ...
                        for dx in range(-1, 2):
                            for dy in range(-1, 2):
                                ...
                                if (
                                    ...
                                    adj_cell = self.grid.cells[nx][ny]
                                    if (
                                        not adj_cell.is_revealed
                                        and not adj_cell.is_flagged
                                    ):
                                        if self.grid.reveal.reveal_cell(nx, ny):

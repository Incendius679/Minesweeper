...
class Game:
    def __init__(
        self, cell_num: int = 20, mines_count: int = 20, num_flags: int = 20
    ) -> None:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:  # pylint: disable=no-member
                ...
                if event.key == pygame.K_SPACE:  # pylint: disable=no-member
                    ...
                    self.top_bar.timer.is_paused = self.paused
                ...
                if mouse_y > self.top_bar_height:  # Grid area
                    ...
                    if event.button == 1:  # Left click
                        if self.grid.reveal.reveal_cell(cell_x, cell_y):
                            self.handle_game_over()
                        else:
                            self.top_bar.stats.score += 1

                        if self.grid.reveal.check_win_condition():
                            self.handle_win()
                            self.top_bar.stats.score += 1000

                    elif event.button == 3:  # Right click
                        cell = self.grid.cells[cell_x][cell_y]
                        if not cell.is_flagged and self.top_bar.stats.num_flags:
                            cell.toggle_flag()
                            self.top_bar.stats.num_flags -= 1
    ...
    def draw_game_over(self) -> None:
        ...
        game_over_text: pygame.Surface = font.render(
            text="Game Over", antialias=True, color=RED
        )
        rect: pygame.Rect = game_over_text.get_rect(
            center=(self.screen_width // 2, self.screen_height // 2)
        )
    ...
    def draw_win(self) -> None:
        font: Font = pygame.font.SysFont(name=None, size=72)
        game_over_text: pygame.Surface = font.render(
            text="You Win", antialias=True, color=GREEN
        )
        rect: pygame.Rect = game_over_text.get_rect(
            center=(self.screen_width // 2, self.screen_height // 2)
        )
